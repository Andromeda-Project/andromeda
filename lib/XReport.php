...
class XReport extends XTable2
{
    ...
    public function RepStandardBreakLevelsInit(&$abreaks, $row)
    {
    ...
    public function RepStandardBreakLevels(&$abreaks, $row, $force=false)
    {
    ...
    public function RepStandardBreak(&$abreaks, $lev, $row)
    {
    ...
    public function RepStandardRowsToLevels(&$abreaks, $row)
    {
        foreach ($abreaks as $level=>$abreak) {
            foreach ($abreak['data'] as $colname=>$colinfo) {
                ...
                case '-NA-':
                    break;
                case 'AVG':  $cnt++;
                case 'SUM':  $val+=$row[$colname];
                    break;
                case 'COUNT':$val++;
                    break;
                case 'MIN':  $val=max($val, $row[$colname]);
                    break;
                case 'MAX':  $val=min($val, $row[$colname]);
                    break;
                }
    ...
    public function RepStandardQuery()
    {
        ...
        foreach ($this->rows_col as $colinfo) {
            ...
            $aTemp[]=str_pad(
                $x, $colinfo['dispsize'], ' ', STR_PAD_RIGHT
            );
        }
    ...
    // =================================================================
    public function ehFlushLine($increment=true)
    {
    ...
    public function PlaceCell($xpos, $value, $class='')
    {
        $xpos='';
        $class='';
        $this->aLine[]=$value;
