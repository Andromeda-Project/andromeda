...
function input($colinfo, &$tabLoop = null, $options = array()) {
    ...
    $x6 = vgfGet('x6');
    if ($x6) {
        $x6options = array('onkeyup' => 'x6inputs.keyUp(this,event)', 'onkeydown' => 'return x6inputs.keyDown(this,event)', 'onfocus' => 'x6inputs.focus(this)', 'onblur' => 'x6inputs.blur(this)', 'attributes' => array('xClassRow' => 1), 'tabIndex' => true);
        $x6options['attributes'] = array_merge(array('xClassRow' => 1), arr($options, 'attributes', array()));
    ...
    if (!$xRoIns) {
        ...
        $autos = array('SUM', 'COUNT', 'FETCH', 'DISTRIBUTE', 'SEQUENCE', 'TS_INS', 'TS_UPD', 'UID_INS', 'UID_UPD', 'EXTEND', 'MIN', 'MAX');
    ...
    if (arr($colinfo, 'x6view', '') == 'window') {
        $input = html('a');
        $input->setHtml('View');
        $input->hp['href'] = "javascript:x6inputs.viewClob(this,'$table_id','$column_id')";
        return $input;
    }
    if (trim($type_id) == 'mime-f' && $x6) {
        ...
        $option->hp['value'] = 'N';
    } elseif (($type_id == 'text' || $type_id == 'mime-h' || $type_id == 'mime-h-f') && arr($options, 'forceinput', false) == false) {
        $input = html('textarea');
    ...
    $input->hp['xTabGroup'] = arr($options, 'xTabGroup', arr($options, 'tabGroup', 'tgdefault'));
    ...
    if ($input->htype == 'input') {
        ...
        $input->hp['size'] = min(arr($colinfo, 'dispsize', 30), OptionGet('dispsize', 30));
        ...
        switch ($colinfo['type_id']) {
            case 'numb':
                if ($colinfo['colscale'] == 0) {
                    ...
                    $inputMask = str_repeat('9', $left) . '.' . str_repeat('9', $colinfo['colscale']);
                }
                break;
    ...
    // Look for "on" options
    $list = array('onchange', 'onkeyup', 'onkeydown', 'onkeypress', 'onfocus', 'onblur', 'onmouseover', 'onmouseout', 'onclick');
...
function inputsTabLoop(&$tabLoop, $options = array()) {
    ...
    for ($x = 0; $x <= $last; $x++) {
        $tabLoop[$x]->hp['onkeypress'] = 'return x4.stdlib.inputKeyPress(event,this)';
        
        $tabLoop[$x] = inputFixupByType($tabLoop[$x]);
        
        if ($xpId <> '') {
            $tabLoop[$x]->hp['onfocus'] = "\$a.byId('$xpId').zLastFocusId = this.id;" . "u.bb.vgfSet('focus',this);";
        }
    }
}
            ...
            switch ($type_id) {
                    ...
                    if (substr($tcv, 0, 1) == '>' || substr($tcv, 0, 1) == '<') {
                        ...
                        if (strlen(trim($tcv)) > 0) {
                            if (gp('x6exactPre', false)) {
                                ...
                                $new = "(LOWER($c) >= LOWER($sbeg)" . " AND LOWER($c) < LOWER($send))";
                            }
                        }
                    }
                    break;
                ...
                case 'numb':
                    if (substr($tcv, 0, 1) == '>' || substr($tcv, 0, 1) == '<') {
                        $tcv = str_replace('%', '', $tcv);
                        if (strlen($tcv) > 1) {
                            $new = $c . substr($tcv, 0, 1) . SQLFN(floatval(substr($tcv, 1)));
                        }
                    } elseif (strpos($tcv, $dash) !== false && $ignore_dashes <> 'Y') {
                        list($beg, $end) = explode($dash, $tcv);
                        if (strlen($beg) > 0 && strlen($end) > 0) {
        ...
        */
        function mosLoadModules($name, $arg1 = null) {
            ...
            if (function_exists('appLoadModules')) {
                ...
                echo 'Could not find appLoadModules() or tmpLoadModules(). ' . " This message sponsored by module '$name'";
            }
            return $ret;
        }
...
?>
    <ul class='right'>
        <?php
            echo $extra ?>
        <li><a href='?st2logout=1'>Logout <?php
            echo SessionGet('UID') ?></a></li>
    </ul>
    <?php
            return false;
        }
        ...
        function hLinkImage($pic, $alt, $var, $val, $enabled) {
            ...
            if ($enabled) {
                return "<a href=\"javascript:formPostString('" . $hparms . "')\">" . "<img src=\"images/$pic.$ext\" border=0 alt=\"" . $alt . "\"" . " onmouseover=\"this.src='images/$pic-over.$ext'\" " . " onmouseout=\"this.src='images/$pic.$ext'\"> " . "</a>";
            } else {
        ...
        */
        function ddTable($table_id) {
            ...
            if (SessionGet('TABLEPERMSMENU') != '') {
                $tabdd['perms']['menu'] = in_array($table_id, SessionGet('TABLEPERMSMENU'));
            } else {
                $tabdd['perms']['menu'] = false;
            }
            if (SessionGet('TABLEPERMSSEL') != '') {
                $tabdd['perms']['sel'] = in_array($table_id, SessionGet('TABLEPERMSSEL'));
            } else {
                $tabdd['perms']['sel'] = false;
            }
            if (SessionGet('TABLEPERMSINS') != '') {
                $tabdd['perms']['ins'] = in_array($table_id, SessionGet('TABLEPERMSINS'));
            } else {
                $tabdd['perms']['ins'] = false;
            }
            if (SessionGet('TABLEPERMSUPD') != '') {
                $tabdd['perms']['upd'] = in_array($table_id, SessionGet('TABLEPERMSUPD'));
            } else {
                $tabdd['perms']['upd'] = false;
            }
            if (SessionGet('TABLEPERMSDEL') != '') {
                $tabdd['perms']['del'] = in_array($table_id, SessionGet('TABLEPERMSDEL'));
            } else {
        ...
        */
        ...
        function userPrefsLoad() {
            ...
            if ($table == '') {
                ...
                $row = SQL_OneRow("select * from $table where user_id=" . "'" . SessionGet("UID") . "'");
            }
        ...
        function hDateBuild($item) {
            ...
            if (!is_array($item)) {
                ...
                return hDateBuild($item['left']) . $item['mid'] . hDateBuild($item['right']);
            }
        }
...
?>
   ...
   <table <?php
     ...
     <tr>
      <td <?php
                echo $hclass ?>>User Login:</td>
     </tr>
     <tr>
      <td ><input type="text" name="loginUID" <?php
                echo $hValue ?>
      ...
      <div class="btn-group">
        ...
        */
        function httpHeadersForDownload($filespec, $attachment = false) {
            ...
            header('Content-disposition: ' . $dispo . '; filename="' . basename($filespec) . '"');
        ...
        function UserAdd($UID, $PWD = '', $email = '', $user_active = 'Y') {
            ...
            SQL("INSERT INTO users (user_id,member_password,email) " . " values ('$UID','$PWD','$email')");
            if (Errors()) {
                scDBConn_Pop();
                return false;
            }
   ...
   <?php
        ...
        */
        ...
        function CellGet($table_id, $val, $col) {
            ...
            if ($retval == '') {
                $table = DD_TableRef($table_id);
                $sq = "SELECT $col FROM $table_id " . " WHERE " . $table['pks'] . ' = ' . SQL_Format($table['flat'][$table['pks']]['type_id'], $val);
        ...
        function HTML_Format($t, $v) {
            ...
            function hTableSortable($table_id, $cols, $class = 'dhead') {
                ...
                $retval = '<table class="table table-bordered table-hover table-striped table-condensed">' . "\n";
                $retval.= '<thead>';
                foreach ($cols as $colname => $colcap) {
                    $href = "javascript:SaveAndPost('" . $hid . "','" . $colname . "')";
                    
                    $retval.= '<th class="' . $class . '">' . '<a href="' . $href . '">' . $colcap . '</a>' . '</th>';
                }
                $retval.= '</thead>';
                return $retval;
            }
            ...
            function FindAccessKey($caption) {
                ...
                if ($x !== false) {
                    ...
                    $caption = ($x == 0 ? '' : substr($caption, 0, $x)) . "<u>" . substr($caption, $x + 1, 1) . "</u>" . substr($caption, $x + 2);
                }
            ...
            function getRegexOptionVal($matches) {
                ...
                $ret = '';
                if (count($result) > 0) {
                    $ret = preg_replace_callback("/%%(.+?)%%/", "getRegexOptionVal", $result['variable_value']);
                }
                return $ret;
            }
            ...
            function DD_RowsAsTable($table_id, $colcolname, $colcoldesc, $colinfo) {
                ...
                $flat = array();
                $results = SQL("Select " . $colcolname . " as column_id," . $colcoldesc . " as coldesc " . "  from $table_id " . " ORDER BY $colcolname ");
            ...
            function CheckTextDate($input) {
                ...
                if (Count($arr) != 3) {
                    ...
                    return checkdate($arr[0], intval($arr[1]), intval($arr[2]));
                }
            }
            ...
            function rff_OneCol($colinfo, $colname, $tcv) {
                ...
                foreach ($values as $tcv) {
                    ...
                    if (substr($tcv, 0, 1) == '>' || substr($tcv, 0, 1) == '<') {
                        ...
                        $new = $colname . ' BETWEEN ' . SQL_Format($tid, $beg) . ' AND ' . SQL_Format($tid, $end);
                    } else {
                        if (!isset($aStrings[$tid]) && strpos($tcv, '%') !== false) {
                            ...
                            if (substr($tcsql, 0, 1) != "'" || $tid == 'date' || $tid == 'dtime') {
                                ...
                                $new = "(    LOWER($colname) like '" . strtolower($tcsql) . "%'" . "\n          OR " . "     UPPER($colname) like '" . strtoupper($tcsql) . "%')";
                            }
                        }
                    }
                    $sql_new[] = "($new)";
                ...
                foreach ($rows as $row) {
                    ...
                    foreach ($inputs as $colname => $allowread) {
                        if ($allowread <> 'Y') {
                            ...
                            $input_name = AddControl($table_id, $row['skey'], $colname, $row[$colname]);
            ...
            function hWidget($type_id, $name, $value = '', $opts = array()) {
                ...
                $col = array('parms' => array('name' => $name, 'value' => $value, 'type' => 'textbox', 'maxlength' => strlen($value), 'size' => strlen($value) + 1, 'tabindex' => hpTabIndexNext()), 'hparms' => '', 'html' => '', 'input' => 'input', 'hinner' => '');
                ...
                switch ($type_id) {
                    case 'cbool':
                        ...
                        $col['parms']['value'] = 'Y';
                         // checkboxes must always be 'Y'
                        $col['hinner'] = $value;
                         // Assign value as caption
                        $col['parms']['type'] = 'checkbox';
                ...
                $inp = $col['input'];
                $col['html'] = "<" . $inp . $hparms . ">" . $col['hinner'] . "</" . $inp . ">";
                
                // If its a date, put popup next to it
                if ($type_id == 'date') {
                    $cname = $col['parms']['name'];
                    $col['html'].= "&nbsp;&nbsp;" . "<img src='clib/dhtmlgoodies_calendar_images/calendar1.gif' value='Cal'
            ...
            */
            ...
            function ahInputsComprehensive(&$table, $mode, $row = array(), $projection = '', $opts = array()) {
                ...
                foreach ($acols as $colname) {
                    ...
                    $writable = isset($ddmatches[$colname]) ? false : DDColumnWritable($colinfo, $mode, $value);
                    $propti = $writable ? hpTabIndexNext() : 999;
                    
                    $ahcols[$colname] = array('writeable' => $writable, 'hparms' => '', 'hinner' => '', 'hright' => '', 'html' => '', 'errors' => '', 'parms' => array('value' => $value, 'type' => 'textbox', 'class' => 'inp-' . $mode, 'name' => $name, 'size' => min($hpsize, $colinfo['dispsize']) + 1, 'maxlength' => $colinfo['dispsize'], 'tabindex' => $propti));
                    if (isset($opts['dispsize'])) {
                        $ahcols[$colname]['parms']['size'] = min($ahcols[$colname]['parms']['size'], $opts['dispsize']);
                    }
                    ...
                    if (count($colerrsx) > 0) {
                        $ahcols[$colname]['errors'] = "<span class=\"x2columnerr\">" . implode("<br/>", $colerrsx) . "</span>";
                    }
                    
                    // refinement: PK caps
                    if ($table["capspk"] == "Y" && $colinfo["primary_key"] == "Y") {
                        $ahcols[$colname]['parms']['onBlur'] = "javascript:this.value=this.value.toUpperCase();\" ";
                    }
                    
                    // KFD 1/12/07  If an ajax callback column, put that in.
                    if ($ajax_page <> '') {
                        if (in_array($colname, $ajax_columns)) {
                            $ahcols[$colname]['parms']['onChange'] = "javascript:AjaxCol('$colname',this.value);";
                        }
                    }
                ...
                foreach ($acols as $colname) {
                    ...
                    switch ($coltype) {
                        ...
                        case 'mime-x':
                            $col['html'] = hImageFromBytes($table_id, $colname, $row[$table['pks']], $row[$colname]);
                        ...
                        case 'mime-h':
                            ...
                            //$col['html']=
                            $col['input'] = 'textarea';
                            $col['parms']['rows'] = isset($colparms[$colname]['rows']) ? $colparms[$colname]['rows'] : '10';
                            $col['parms']['cols'] = isset($colparms[$colname]['cols']) ? $colparms[$colname]['cols'] : '50';
                    ...
                    if ($table_id_fko <> '' && $colinfo['type_id'] <> 'date') {
                        ...
                        if (isset($columnoverrides[$colname]['fetchrow'])) {
                            $col['parms']['onblur'] = "FetchRow('$table_id_fko','" . $col['parms']['name'] . "')";
                        }
                        ...
                        if ($col['writeable']) {
                            ...
                            if ($fkdisplay <> 'dynamic') {
                                ...
                                $col['input'] = 'select';
                                $col['hinner'] = hOptionsFromTable($table_id_fko, ArraySafe($row, $colname), '', $matches);
                                ...
                                $col['parms']['size'] = 10;
                                $col['parms']['value'] = hValueForSelect($table_id_fko, $col['parms']['value']);
                            }
                        }
                    }
                }
                ...
                foreach ($acols as $colname) {
                    ...
                    $ahcols[$colname]['html'] = "<" . $inp . $hparms . ">" . $col['hinner'] . "</" . $inp . ">";
                    
                    // Now stick a date next to every last one of them
                    if ($table['flat'][$colname]['type_id'] == 'date') {
                        if ($col['writeable']) {
                            $cname = $col['parms']['name'];
                            $ahcols[$colname]['html'].= "&nbsp;&nbsp;" . "<img src='clib/dhtmlgoodies_calendar_images/calendar1.gif' value='Cal'
                      onclick=\"displayCalendar(ob('$cname'),'mm/dd/yyyy',this,true)\">";
                        }
                    }
            ...
            function ajsFromDD($table, $prefix) {
                ...
                foreach ($table['fk_parents'] as $fkp) {
                    $fk = $table['table_id'] . $fkp['prefix'] . '_' . $fkp['table_id_par'] . '_' . $fkp['suffix'];
                    ...
                    if (isset($table['FETCHDIST'][$fk])) {
                        ...
                        $cm = "ajaxFetch(" . "'" . $fkp['table_id_par'] . "'" . ",'$prefix'" . ",'" . $ddpar['pks'] . "'" . ",'" . implode(',', $acontrols) . "'" . ",'" . implode(',', $acolumns) . "'" . ")";
...
?>
   <textarea rows=15 cols=50 style="width: 100%" name="<?php
                echo $name ?>" id="<?php
                echo $name ?>"><?php
                echo $value ?></textarea>
    <script language="javascript">
      editor_generate('<?php
                echo $name ?>'); // field, width, height
    </script>
         ...
         WHERE " . $table['pks'] . " = " . SQLFC($pkval);
            ...
            function hSelectFromAA($aa, $name, $selected = '', $extra = '') {
                ...
                foreach ($aa as $value => $caption) {
                    ...
                    $hOpts.= '<OPTION VALUE="' . $value . '" ' . $hSELECTED . '>' . $caption . '</OPTION>';
                }
            ...
            function HTMLE_Table(&$dbrows, $args = array()) {
                ...
                if (isset($args['cols'])) {
                    ...
                    $cols = array();
                    foreach ($x as $col) {
                        if (!is_numeric($col) && !isset($colsx[$col])) {
                            $cols[$col] = $col;
                        }
                    }
                }
                    ...
                    foreach ($cols as $colname => $colinfo) {
                        ...
                        if (is_array($colinfo)) {
                            if (isset($colinfo['hlink'])) {
                                $hlink = $colinfo['hlink'];
                                $inner = HTMLE_A_ARRAY($row[$colname], array('gp_page' => $hlink['gp_page'], 'txt_' . $hlink['valpk'] => trim($row[$hlink['valpk']])));
                            }
                        }
            ...
            function ehXMLDoc($feed_id, $atts, $xmlresult) {
                ...
                if ($atts['result'] == 'fail') {
                    $feed_id = 'error';
                    $xmlresult = array('error' => array(array('attributes' => $atts)));
                } else {
            ...
            */
            ...
            ");
                ...
                if ($ahcol['formshort'] == 'char' || $ahcol['formshort'] == 'varchar' || $ahcol['formshort'] == 'text') {
                ...
                if ($ahcol['table_id_fko'] <> '' && $ahcol['fkdisplay'] <> 'dynamic') {
                    ...
                    if (count($fkpks) > 1) {
                        ...
                        if (trim($colname) == trim($fkpks[0])) {
                            ...
                            $pull = false;
                            $innerHTML = "<option SELECTED value=\"$colvalue\">$colvalue</option>";
                        }
                    }
                    
                    // Pull the options
                    if ($pull) {
                        $innerHTML = hOptionsFromTable($ahcol['table_id_fko'], $colvalue, '', $matches, $dist);
                    }
            ...
            function aColsModeProjCalcRowColumn(&$chaincalc) {
                ...
                return "if(ob('--NAME-PREFIX--')) ob('--NAME-PREFIX--$colname').value=" . implode(' ' . trim($chaincalc['funcoper']) . ' ', $expr);
            }
            ...
            function aColInfoFromDDTable(&$table, &$retval) {
                ...
                foreach ($table['fk_parents'] as $fkp) {
                    ...
                    if (isset($table['FETCHDIST'][$fk])) {
                        ...
                        $details = array('table_id_par' => $fkp['table_id_par'], 'commapklist' => $fkp['cols_par'], 'commafklist' => $fkp['cols_chd'], 'controls' => implode(',', $acontrols), 'columns' => implode(',', $acolumns));
...
?>
    ...
    <?php
            ...
            function jsInclude($file, $comments = '', $immediate = false) {
                ...
                $newEntry = array('file' => $file, 'comments' => $comments, 'immediate' => $immediate);
                if ($immediate == 'Y') {
    ...
    <?php
            ...
            function SQL_FKJOIN($pks, $fkey_suffix, $child = "", $parent = "") {
                ...
                foreach ($pksarr as $colname) {
                    $retval.= ListDelim($retval, " AND ") . $child . "." . $colname . $fkey_suffix . " = " . $parent . "." . $colname;
                }
                return $retval;
            }
            ...
            function SQLX_Delete($table_id, $row) {
                ...
                foreach ($row as $colname => $colval) {
                    $awhere[] = $colname . ' = ' . SQL_Format($table_dd['flat'][$colname]['type_id'], $row[$colname]);
                }
